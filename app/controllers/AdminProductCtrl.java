...
public class AdminProductCtrl extends Controller {
    ...
    @Transactional
    public Result restockProduct(Long id) {
        // Call delete method
        Product.find.ref(id).setStock(Product.find.ref(id).getStock()+20);
        Product.find.ref(id).update();
        // Add message to flash session
        flash("success", "Product has been restocked");
        // Redirect home
        return redirect(routes.AdminProductCtrl.index());
    }

    @Transactional
    public Result restockAllProducts() {
        // Call delete method
        List<Product> products = Product.findAll("");
        for(Product p : products){
            if(p.getStock()<=0){
                Product.find.ref(p.getId()).setStock(Product.find.ref(p.getId()).getStock()+20);
                Product.find.ref(p.getId()).update();
            }
        }
        // Add message to flash session
        flash("success", "Products have been restocked");
        // Redirect home
        return redirect(routes.AdminProductCtrl.index());
    }

    public Result getMessages() {
        String name = "Issues Raised";
        Administrator administrator = (Administrator) getCurrentUser();

        return ok(issueResults.render(env, administrator, administrator.getMessages(), name));
    }
